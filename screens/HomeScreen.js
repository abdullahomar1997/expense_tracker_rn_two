...
const HomeScreen = ({navigation}) => {
  ...
  useEffect(() => {    
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        const transactionsData = snapshot.docs.map((doc) => ({id: doc.id,data: doc.data()}));
        setTransactions(transactionsData);
        
        const incomeTotal = snapshot.docs.map((doc) => doc.data()?.email === auth.currentUser.email
          && doc.data()?.type == 'income' ? doc.data().price : 0)
        setTotalIncome(incomeTotal);
        
        const expenseTotal = snapshot.docs.map((doc) => doc.data()?.email === auth.currentUser.email 
          && doc.data()?.type == 'expense' ? doc.data().price : 0)
